...
class AssetController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($existingAsset && $existingAsset->trashed()) {
            ...
            return redirect()
                ->route('assets.index')
                ->with('success', "Aset {$validatedData['name']} berhasil dipulihkan dan diperbarui.");
        }
        ...
        return redirect()
            ->route('assets.index')
            ->with('success', "Aset {$validatedData['name']} berhasil ditambahkan.");
    }
...
{
    $categories = Category::all();
    $locations = AssetLocation::all(); // Changed from vendors

    return view('master.assets.edit', compact('asset', 'categories', 'locations')); // Changed vendors to locations
}
    ...
    private function validateAsset(Request $request, $id = null)
{
    ...
    $messages = [
        ...
        'location_id.required' => 'Lokasi aset wajib diisi!',
        'location_id.exists' => 'Lokasi aset tidak valid.',
